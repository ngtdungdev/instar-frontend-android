...
class DetailMySavedPostActivity : AppCompatActivity() {
    private lateinit var binding: ActivityDetailMySavedPostBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_detail_my_saved_post)
        ...
        binding = ActivityDetailMySavedPostBinding.inflate(layoutInflater)
        ...
        lifecycleScope.launch {
            val postDataList = user?.let { getPosts(it.id) }
    ...
    private suspend fun getPosts(id: String): ArrayList<Post> {
        ...
        val response = try {
            val response = userService.getTimelineMySavedPosts(id).awaitResponse()
            response
